...
import { Conclusao } from "@/components/s5/conclusao";
import {
  IconArrowLeft,
  IconCoin,
  IconHouse,
...
function S5Content() {
  ...
  const protocoloEmitidoEm = useRegularizaStore((s) => s.protocoloEmitidoEm);
  const canalEnvio = useRegularizaStore((s) => s.canalEnvio);
  const contatoEnvio = useRegularizaStore((s) => s.contatoEnvio);
        ...
        <section className="pt-8 pb-5 border-b border-kr-deep/10">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-baseline sm:justify-between sm:gap-6">
            <h1 className="font-display font-bold text-[28px] md:text-[32px] lg:text-[38px] leading-[1.1] text-kr-deep tracking-[-1.0px] m-0">
              {concluido ? "Acordo finalizado" : "Como você quer pagar?"}
            </h1>
          ...
          {!concluido && <AvisoAtivacao proxVencto={proxVencto} />}
        </section>
